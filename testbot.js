...
bot.login("NTg5MTMxNzI2NzE5MDI1MTgy.XhURWQ.y0wJwd4BTiwLFXvvLu2tRpCwtyk")
...
/////////////////////////////////////////////////////////////////////// MOVEMENT COMMANDS //////////////////////////////////////////////////////////////////////////////
	else if (primaryCommand == "north")
	{
			...
			if (replacement == "01")
			{
				message.channel.send("You cannot move in that direction from here.").then(msg => {msg.delete(6000)}).catch
			}
			...
			{
				message.channel.send("You cannot walk that direction. A boma wall blocks your path").then(msg => {msg.delete(6000)}).catch
			}
			else if (str == "b02" || str == "c02" || str == "d02" || str == "e02" || str == "i09" || str == "j09" || str == "k09" || str == "l09")
			{
				message.channel.send("You cannot leave the boma this way").then(msg => {msg.delete(6000)}).catch
			}
			
			//make sure player can't walk into challenge square
			else if (str == "g08")
			{
				message.channel.send("That crosses into the other tribe's tribal territory. Production says you shouldn't do that").then(msg => {msg.delete(6000)}).catch
			}
	...
	else if (primaryCommand == "east")
	{
			...
			if (replacement == 'm')
			{
				message.reply("You cannot move in that direction from here.").then(msg => {msg.delete(6000)}).catch
			}
			...
			{
				message.channel.send("You cannot leave the boma at night").then(msg => {msg.delete(6000)}).catch
			}
			else if (str == "h10")
			{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////// IDOL COMMANDS //////////////////////////////////////////////////////////////////////////////////////////////
	else if (primaryCommand == "reachIntoFire")
	{
		...
		if (str == "j10")
		{
			...
			{
				message.channel.send("What in the hell do you think you're doing?! The fire is still raging and you burned your hand. Congrats dingus").then(msg => {msg.delete(6000)}).catch
	...
	else if (primaryCommand == "storeWood")
	{
		...
		if (argument)
		{
			...
			{
				message.channel.send("You are not at your tribe's wood storage location").then(msg => {msg.delete(6000)}).catch
			}
		}
		
		else
		{
			message.channel.send("Please include how much wood you want to store").then(msg => {msg.delete(6000)}).catch
		}
	}
	...
	else if (primaryCommand == "storeWater")
	{
		...
		if (argument)
		{
			...
			{
				message.channel.send("You are not at your tribe's water storage location").then(msg => {msg.delete(6000)}).catch
			}
		}
		
		else
		{
			message.channel.send("Please include how much wwater you want to store").then(msg => {msg.delete(6000)}).catch
		}
	}
	...
	else if (primaryCommand == "storeFood")
	{
		...
		{
			message.channel.send("Please include how much food you want to store").then(msg => {msg.delete(6000)}).catch
		}
	}
	...
	else if (primaryCommand == "stokeFire")
	{
		if (str == "j10")
		{
			...
			message.channel.send("You stoke up the fire and now it is raging again! This should last a good amount of time").then(msg => {msg.delete(6000)}).catch
		}
...
function lockout(user, message, lockoutTime)
{
	...
	message.channel.send("@" + message.member.nickname + " you are now locked for " + stuckTime + " minutes and cannot move from this location").then(msg => {msg.delete(9000)}).catch
	user.removeRole('617816630721904654')
	var interval = setTimeout (function () 
	{
            // use the message's channel (TextChannel) to send a new message
            message.channel.send("@" + message.member.nickname + " you can now move again").then(msg => {msg.delete(9000)}).catch
